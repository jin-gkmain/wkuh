...
export default function MobileHosIndexPage() {
  ...
  const handlePaperweightClick = () => {
    router.push(`/mobile/${hos}/paperweight`); // 예시 경로
  };
  ...
  return (
    <MobileLayout isNotMain={true}>
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', m: 2, mt: 5, pb: '100px' }}>
        <Stack sx={{ width: '100%', alignItems: 'center' }} direction="column" spacing={6} >
          <Box sx={{ width: '100%', textAlign: 'center' }}>
            <Typography variant="h4" sx={{ fontWeight: 'bold' }}>
              {langFile[lang]?.MOBILE_INDEX_TITLE || `Welcome to ${hos}`}
            </Typography>
            <Typography variant="subtitle1" color="text.secondary" pl={4} pr={4} pt={1} pb={1}>
              {langFile[lang]?.MOBILE_INDEX_DESC || "Please select an option below."}
            </Typography>
          </Box>
          ...
          */}
          ...
          <ButtonCard 
            title={langFile[lang]?.MOBILE_INDEX_PAPERWEIGHT_TITLE || "Fill Questionnaire"} 
            content={langFile[lang]?.MOBILE_INDEX_PAPERWEIGHT_DESC || "Complete your medical questionnaire."}
            action={handlePaperweightClick} 
            icon={<LibraryBooksIcon/>}
            media={MobilePaperweightImage.src} // 이미지 경로 확인 및 수정 필요
            buttonText={langFile[lang]?.MOBILE_INDEX_PAPERWEIGHT_BUTTON || "Start"} 
          />
          <ButtonCard 
            title={langFile[lang]?.MOBILE_INDEX_CHATBOT_TITLE || "Chat Consultation"} 
            content={langFile[lang]?.MOBILE_INDEX_CHATBOT_DESC || "Get help via our chatbot."}
