...
export default function DropFileInput({
  ...
  const checkExt = (fileName: string) => {
    const filetypes =
      /jpeg|jpg|png|gif|pdf|xls|ppt|xlsx|pptx|hwp|hwpx|doc|docs|zuo/;
        ...
        >
          {
            labelText &&
              (short
                ? langFile[lang].ATTACHED_FILE_PLACEHOLDER2
                : langFile[lang].ATTACHED_FILE_PLACEHOLDER) // 파일 추가 혹은 여기로 드래그 (20MB 이하, 최대 10개)
          }
          <Clip className="clip" />
        </label>
      </div>
