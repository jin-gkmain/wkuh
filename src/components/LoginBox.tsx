  ...
  return (
    <form className="login-box relative flex flex-col" onSubmit={onSubmit}>
      ...
      <h2 className="sub-title text-center">
        {
          type === "login"
            ? langFile[webLang].LOGIN_TITLE_TEXT // 원격협진 서비스 접속
            : type === "findPsw"
            ? isSentMail
              ? langFile[webLang].LOGIN_CHECK_EMAIL_TITLE_TEXT // 이메일을 확인해 주세요
              : langFile[webLang].LOGIN_FIND_PSW_TITLE_TEXT // 비밀번호 찾기
            : langFile[webLang].LOGIN_RESET_PSW_TITLE_TEXT // 새 비밀번호 입력
        }
      </h2>
      ...
      {type === "findPsw" && isSentMail && (
        <p className="sent-mail-text font-bold text-center">
          <span>
            {langFile[webLang].LOGIN_CHECK_EMAIL_DESC_TEXT1}{" "}
            {/** 비밀번호 재설정 메일을 */}
            <br /> {loginInfo.email}{" "}
            {langFile[webLang].LOGIN_CHECK_EMAIL_DESC_TEXT2}
            {/** 으로 보냈습니다. */}
          </span>
        </p>
      )}
        ...
        {(type === "login" || (type === "findPsw" && !isSentMail)) && (
          <div className="input-col-wrap">
            <label htmlFor="email" className="font-bold label">
              {langFile[webLang].LOGIN_EMAIL_LABLE_TEXT}
              {/** 이메일 주소 */}
            </label>
            <input
              type="text"
        ...
        {(type === "login" || type === "reset") && (
          <div className="input-col-wrap">
            <label htmlFor="password" className="font-bold label">
              {type === "login"
                ? langFile[webLang].LOGIN_PASSWORD_LABLE_TEXT
                : langFile[webLang].LOGIN_NEW_PSW_LABEL_TEXT}
            </label>
            <input
              ref={inputRef}
      ...
      <div>
        ...
        >
          {
            type === "login"
              ? langFile[webLang].LOGIN_BUTTON_TEXT // 로그인하기
              : type === "findPsw"
              ? isSentMail
                ? langFile[webLang].LOGIN_SEND_AGAIN_BUTTON_TEXT // 이메일 다시 보내기
                : langFile[webLang].LOGIN_FIND_PSW_BUTTON_TEXT // 비밀번호 재설정 메일 발송
              : langFile[webLang].LOGIN_CP_RESET_PSW_BUTTON_TEXT // 비밀번호 재설정 완료
          }
        </button>
        {type === "login" && (
          <p className="text-center">
            <Link href={"/psw/find"} className="find-psw font-bold">
              {langFile[webLang].LOGIN_FORGET_PASSWORD_TEXT}{" "}
              {/** 비밀번호를 잊으셨나요? */}
            </Link>
          </p>
        )}
        {type !== "login" && (
          <p className="text-center">
            <Link href="/" className="find-psw font-bold">
              {langFile[webLang].LOGIN_RETURN_LOGIN_TEXT}
              {/** 로그인으로 돌아가기 */}
            </Link>
          </p>
        )}
...
function AccountInfoContactModalBox({
  ...
  return (
    <div className="account-info-contact-modal-box">
      <h2 className="title">{langFile[webLang].ACCOUNT_CONTACT_MODAL_TITLE}</h2>

      <div className="contents">
        <div className="main-contents input-row-wrap justify-center">
          <div className="input-col-wrap">
            <span>
              {langFile[webLang].ACCOUNT_CONTACT_MODAL_ORG_NAME} {/** 기관명 */}
            </span>
            <span>
              {langFile[webLang].ACCOUNT_CONTACT_MODAL_EMAIL} {/** 문의메일 */}
            </span>
            <span>
              {langFile[webLang].ACCOUNT_CONTACT_MODAL_TEL} {/** 연락처 */}
            </span>
          </div>

          <div className="input-col-wrap">
            <span>
              {langFile[webLang].ACCOUNT_CONTACT_MODAL_ORG_NAME_VALUE}
              {/** 원광대병원 */}
            </span>
        ...
        <p>
          {langFile[webLang].ACCOUNT_CONTACT_MODAL_DESC}
