...
export default function ResetPswModalBox({
  ...
  const handleSubmit = async (ev?: FormEvent) => {
    ...
    if (password.value !== rePassword.value)
      return alert(langFile[webLang].EDIT_PSW_NOT_MATCH_ALERT_TEXT); // 비밀번호가 서로 일치하지 않습니다.
    ...
    if (data.result === "OK") {
      onComplete(true);
    } else alert(langFile[webLang].EDIT_PSW_FAIL_ALERT_TEXT); // 비밀번호 수정에 실패하였습니다.
  };
  return (
    <div className="reset-psw-modal">
      <ModalFrame
        onComplete={handleSubmit}
        title={langFile[webLang].EDIT_PSW_MODAL_TITLE} // 비밀번호수정
        onClose={onClose}
        completeBtnText={
          webLang === "en"
            ? langFile[webLang].EDIT_PSW_MODAL_COMPLETE_BUTTON_TEXT
            : ""
        }
      >
            ...
            <section className="flex-1">
              <div className="input-col-wrap">
                <label htmlFor="email" className="label">
                  {langFile[webLang].EDIT_PSW_MODAL_EMAIL_LABEL_TEXT}
                  {/**이메일(ID)*/}
                </label>
                <input
                  value={userInfo?.u_id}
            ...
            <section className="flex-1 input-col-wrap">
              <div className="input-col-wrap">
                <label htmlFor="password" className="label flex">
                  {langFile[webLang].EDIT_PSW_MODAL_NEW_PSW_TEXT}
                  {/* 비밀번호 */}
                  <span className="meta-info">
                    *
                    {langFile[webLang].EDIT_PSW_MODAL_PSW_CONSTRAINT_LABEL_TEXT}
                    {/* *영문과 숫자만 입력 가능합니다. */}
                  </span>
                </label>
                <input
              ...
              <div className="input-col-wrap">
                <label htmlFor="rePassword" className="label">
                  {langFile[webLang].EDIT_PSW_MODAL_CHECK_PSW_LABEL_TEXT}
                  {/* 비밀번호 확인 */}
                  {webLang === "ko" && (
                    <span className="meta-info">
                      *
                      {
                        langFile[webLang]
                          .EDIT_PSW_MODAL_PSW_CONSTRAINT_LABEL_TEXT
                      }
                      {/* *영문과 숫자만 입력 가능합니다. */}
                    </span>
                  )}
