...
const MobileLayout: React.FC<MobileLayoutProps> = ({ children, isNotMain = false }) => {
      ...
      <div className="mobile-layout"> 
        <div className="mobile-layout-content">
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 2, pb: 0 }}>
            <Box>
            {!isNotMain && <IconButton onClick={() => router.back()}>
              <ArrowBackIcon />
            </IconButton>}
            </Box>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
              <LanguageIcon sx={{ mr: 1 }} />
              <FormControl sx={{ minWidth: 80 }} size="small" variant="standard">
                <Select defaultValue="ko" value={lang} onChange={handleLangChange}>
